...
export const AddPlotForm: FC<IProps> = (props) => {
    ...
    const { nameValue, areaValue, soilValue, plantValue } = defaultValues
        ? defaultValues
        : {
              nameValue: '',
              areaValue: 0,
    ...
    const soilItem = soilTypesData?.filter((item) => item.id === soilValue)
    console.log(soilItem)
    soilTypesData &&
    ...
    if (!soilItem || (soilItem && soilItem.length === 0)) {
        return null
    }
